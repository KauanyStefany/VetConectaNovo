...
def criar_tabela_veterinario() -> bool:
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(veterinario_sql.CRIAR_TABELA)
            return True
...
def obter_por_pagina(limit: int, offset: int) -> list[Veterinario]:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(veterinario_sql.OBTER_VETERINARIO_PAGINADO, (limit, offset))
...
def obter_por_id(id_veterinario: int) -> Optional[Veterinario]:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(veterinario_sql.OBTER_POR_ID, (id_veterinario,))
        row = cursor.fetchone()
        if row is None:
            return None
