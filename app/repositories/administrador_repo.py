...
from app.db.queries import administrador_sql
...
def criar_tabela_administrador() -> bool:
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(administrador_sql.CRIAR_TABELA)
            return True
...
def inserir_administrador(admin: Administrador) -> Optional[int]:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.INSERIR, (admin.nome, admin.email, admin.senha))
        return cursor.lastrowid

def atualizar_administrador(admin: Administrador) -> bool:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.ATUALIZAR, (admin.nome, admin.email, admin.senha, admin.id_admin))
        return cursor.rowcount > 0
    
def atualizar_senha(id_admin: int, nova_senha: str) -> bool:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.ATUALIZAR_SENHA, (nova_senha, id_admin))
        return cursor.rowcount > 0

def excluir_administrador(id_admin: int) -> bool:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.EXCLUIR, (id_admin,))
        return cursor.rowcount > 0

def obter_administradores_paginado(offset: int, limite: int) -> List[Administrador]:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.OBTER_ADMINISTRADORES_PAGINADO, (limite, offset))
...
def obter_administrador_por_id(id_admin: int) -> Optional[Administrador]:
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute(administrador_sql.OBTER_POR_ID, (id_admin,))
        row = cursor.fetchone()
