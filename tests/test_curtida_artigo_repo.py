...
class TestCurtidaArtigoRepo:
    ...
    def test_excluir_curtida_sucesso(self, test_db):
        ...
        # Criar usuário
        usuario = Usuario(0, "Maria", "maria@example.com", "senha123", "123456789", "tutor", None, None, None, None)
        id_usuario = usuario_repo.inserir_usuario(usuario)

        # Criar veterinário
        veterinario = Veterinario(0, "Dr. Lima", "dr.lima@vet.com", "vet123", "987654321", "veterinario", None, None, None, None, "54321-RJ", True, "Veterinário")
        id_veterinario = veterinario_repo.inserir_veterinario(veterinario)
        ...
        # Criar postagem
        postagem = PostagemArtigo(0, id_veterinario, "Doenças em Cães", "Conteúdo", id_categoria, datetime.today().date(), 0)
        id_postagem = postagem_artigo_repo.inserir(postagem)

        # Criar curtida
        curtida = CurtidaArtigo(id_usuario, id_postagem, datetime.today().date())
        curtida_artigo_repo.inserir_curtida_artigo(curtida)

        # Act
        resultado = curtida_artigo_repo.excluir_curtida(id_usuario, id_postagem)
    ...
    def test_excluir_curtida_inexistente(self, test_db):
        ...
        # Act
        resultado = curtida_artigo_repo.excluir_curtida(9999, 9999)

        # Assert
        assert resultado == False, "Exclusão de curtida inexistente deveria retornar False"
    ...
    def test_obter_todos_paginado(self, test_db):
        ...
        # Criar veterinário
        veterinario = Veterinario(0, "Dr. Costa", "dr.costa@vet.com", "vet123", "987654321", "veterinario", None, None, None, None, "11111-SP", True, "Veterinário")
        id_veterinario = veterinario_repo.inserir_veterinario(veterinario)
        ...
        # Criar postagem
        postagem = PostagemArtigo(0, id_veterinario, "Alimentação de Pets", "Conteúdo", id_categoria, datetime.today().date(), 0)
        id_postagem = postagem_artigo_repo.inserir(postagem)

        # Criar 3 usuários e 3 curtidas
        usuarios_ids = []
        for i in range(3):
            usuario = Usuario(0, f"Usuario{i}", f"user{i}@example.com", "senha", "111111111", "tutor", None, None, None, None)
            id_usuario = usuario_repo.inserir_usuario(usuario)
        ...
        # Act
        curtidas = curtida_artigo_repo.obter_todos_paginado(limite=2, offset=0)

        # Assert
        assert len(curtidas) == 2, "Deveria retornar 2 curtidas (limite da paginação)"
        assert all(isinstance(c, CurtidaArtigo) for c in curtidas), "Todos os itens deveriam ser CurtidaArtigo"
    ...
    def test_obter_todos_paginado_vazio(self, test_db):
        ...
        # Act
        curtidas = curtida_artigo_repo.obter_todos_paginado(limite=10, offset=0)
