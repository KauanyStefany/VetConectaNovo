...
class TestVerificacaoCRMVRepo:
    ...
    def test_inserir_verificacao_sucesso(self, test_db):
        ...
        # Assert
        assert id_inserido is not None, "ID da verificação inserida não deveria ser None"
    ...
    def test_atualizar_status_verificacao_sucesso(self, test_db):
        ...
        resultado = atualizar(self.id_veterinario, VerificacaoStatus.APROVADO, self.id_admin)
        ...
        resultado = atualizar(self.id_veterinario, VerificacaoStatus.REJEITADO, self.id_admin)
    ...
    def test_atualizar_verificacao_inexistente(self, test_db):
        ...
        # Assert
        assert resultado == False, "Atualização de verificação inexistente deveria retornar False"
    ...
    def test_excluir_verificacao_inexistente(self, test_db):
        ...
        # Assert
        assert resultado == False, "Exclusão de verificação inexistente deveria retornar False"
    ...
    def test_obter_todos_paginado(self, test_db):
        ...
        verificacoes_ids = []
        for i in range(5):
            verificacao = VerificacaoCRMV(0, self.id_veterinario, self.id_admin, datetime.now(), VerificacaoStatus.PENDENTE)
    ...
    def test_enum_status_valores(self, test_db):
        ...
        statuses = [VerificacaoStatus.PENDENTE, VerificacaoStatus.APROVADO, VerificacaoStatus.REJEITADO]
        ids_verificacao = []
        
        for status in statuses:
            verificacao = VerificacaoCRMV(0, self.id_veterinario, self.id_admin, datetime.now(), status)
