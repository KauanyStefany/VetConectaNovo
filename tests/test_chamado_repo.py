...
class TestChamadoRepo:
    ...
    def test_inserir_chamado_sucesso(self, test_db):
        ...
        # Act
        id_inserido = chamado_repo.inserir_chamado(chamado)
        ...
        chamado_db = chamado_repo.obter_chamado_por_id(id_inserido)
    ...
    def test_atualizar_status_chamado_sucesso(self, test_db):
        ...
        id_chamado = chamado_repo.inserir_chamado(chamado)
        
        # Act - mudar para em andamento
        resultado = chamado_repo.atualizar_status_chamado(id_chamado, ChamadoStatus.EM_ANDAMENTO)
        ...
        resultado = chamado_repo.atualizar_status_chamado(id_chamado, ChamadoStatus.RESOLVIDO)
    ...
    def test_atualizar_status_chamado_inexistente(self, test_db):
        ...
        # Act
        resultado = chamado_repo.atualizar_status_chamado(id_inexistente, ChamadoStatus.RESOLVIDO)
        
        # Assert
        assert resultado == False, "Atualização de chamado inexistente deveria retornar False"
    ...
    def test_excluir_chamado_inexistente(self, test_db):
        ...
        # Act
        resultado = chamado_repo.excluir_chamado(id_inexistente)
        
        # Assert
        assert resultado == False, "Exclusão de chamado inexistente deveria retornar False"

    def test_obter_todos_chamados_paginado(self, test_db):
        """Testa obtenção paginada de chamados"""
        # Arrange
        chamados = [
            Chamado(0, self.id_usuario, self.id_admin, "Erro no login", "Não consigo fazer login", ChamadoStatus.ABERTO, datetime.now()),
            Chamado(0, self.id_usuario, None, "Dúvida", "Como altero meu email?", ChamadoStatus.ABERTO, datetime.now()),
            Chamado(0, self.id_usuario, self.id_admin, "Bug crítico", "Sistema travando", ChamadoStatus.EM_ANDAMENTO, datetime.now()),
            Chamado(0, self.id_usuario, self.id_admin, "Sugestão", "Adicionar modo escuro", ChamadoStatus.RESOLVIDO, datetime.now()),
            Chamado(0, self.id_usuario, None, "Problema resolvido", "Já foi corrigido", ChamadoStatus.RESOLVIDO, datetime.now())
        ]
    ...
    def test_obter_todos_chamados_paginado_vazio(self, test_db):
        ...
        # Act
        chamados = chamado_repo.obter_todos_chamados_paginado(offset=0, limite=10)
    ...
    def test_obter_chamado_por_id_inexistente(self, test_db):
        ...
        # Act
        chamado = chamado_repo.obter_chamado_por_id(id_inexistente)
